...
const ShoeBox = ({name, price, realPrice, rate, image}: PropsType) => {
    ...
    return (
        ...
        >
            ...
            <Flex>
                {
                    Array(rate).fill(null).map((a, index) => {
                        return <Image width={25} height={25} key={index} src={StarActivate} />
                    })
                }
                {
                    Array(5 - rate).fill(null).map((a, index) => {
                        return <Image width={25} height={25} key={index} src={StarDisable} />
                    })
                }
            </Flex>
            <Text fontWeight={800} >À partir de  <span style={{ color: '#D4AA7D' }}>{price} €</span></Text>
            <Text>Prix neuf: <span style={{ textDecoration: 'line-through', fontWeight: 900 }}>{realPrice} €</span> (-{discount}%)</Text>
        </Flex>
    )
}
